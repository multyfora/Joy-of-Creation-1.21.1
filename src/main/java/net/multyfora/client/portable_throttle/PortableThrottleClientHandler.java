...
public class PortableThrottleClientHandler {
    ...
    public static void startBind(BlockPos pos) {
        //LOGGER.debug("[THROTTLE_CLIENT] startBind: pos={}", pos);
        bindTarget = pos;
    }

    // Opens the throttle configuration (frequency) screen
    public static void openScreen() {
        //LOGGER.debug("[THROTTLE_CLIENT] openScreen: opening PortableThrottleScreen (config)");
    ...
    public static void setStrength(int strength) {
        //LOGGER.debug("[THROTTLE_CLIENT] setStrength: lastStrength={} -> {}", lastStrength, strength);
    ...
    public static void tick() {
        ...
        if (player == null || player.isSpectator()) {
            //LOGGER.debug("[THROTTLE_CLIENT] tick: player null or spectator, resetting");
            reset();
            return;
        }
        ...
        if (bindTarget != null) {
            //LOGGER.debug("[THROTTLE_CLIENT] tick: sending bind packet for {}", bindTarget);
        ...
        if (leftDown) {
            /*if(!wasLeftDown) {
                LOGGER.debug("[THROTTLE_CLIENT] tick: left click DETECTED (edge), screen={}", mc.screen);
            }*/
            ...
            if (!wasLeftDown && mc.screen == null) {
                //LOGGER.debug("[THROTTLE_CLIENT] tick: left click edge + no screen -> opening PortableThrottleStrengthScreen");
        ...
        if (lastStrength > 0 && mc.screen == null) {
            if (keepAliveCooldown > 0) {
                ...
                //LOGGER.debug("[THROTTLE_CLIENT] tick: sending KEEPALIVE signal strength={}", lastStrength);
    ...
    private static void reset() {
        //LOGGER.debug("[THROTTLE_CLIENT] reset: called, lastStrength={} bindTarget={}", lastStrength, bindTarget);
        Screen current = Minecraft.getInstance().screen;
        boolean hadScreen = current instanceof PortableThrottleStrengthScreen || current instanceof PortableThrottleScreen;

        if(hadScreen) {
            //LOGGER.debug("[THROTTLE_CLIENT] reset: screen open, sending strength 0 before close");
            PacketDistributor.sendToServer(new PortableThrottleSignalPacket(0));
        } else if (lastStrength > 0) {
            //LOGGER.debug("[THROTTLE_CLIENT] reset: sending strength 0 (shutdown)");
            PacketDistributor.sendToServer(new PortableThrottleSignalPacket(0));
        }

        if(hadScreen) {
            //LOGGER.debug("[THROTTLE_CLIENT] reset: closing open screen");
            Minecraft.getInstance().setScreen(null);
        }
        lastStrength = 0;
        keepAliveCooldown = 0;
        bindTarget = null;
        //LOGGER.debug("[THROTTLE_CLIENT] reset: complete");
    }
}
